...
<div class="max-w-2xl mx-auto space-y-8 py-16">
  <div class="relative pl-10 space-y-10">
    ...
    <div class="absolute inset-y-2 left-[15px] w-[1.5px] bg-ink-ghost/50 z-0"></div>

    <!-- Terra progress line -->
    {#if currentStep > 0}
      <div
        class="absolute top-2 left-[15px] w-[1.5px] bg-terra z-[1] transition-all duration-500"
    ...
    {#each steps as step, i}
      {#if i <= currentStep}
        <div class="relative z-10 flex items-center gap-5 group">
          {#if i < currentStep || isComplete}
            <!-- Completed step: dot -->
            <div class="absolute left-[-37px] p-1.5">
              <div class="w-3 h-3 rounded-full bg-terra shadow-[0_0_0_3px_#fdf6f0,0_0_0_5px_rgba(196,83,58,0.2)]"></div>
            </div>
          {:else}
            <!-- Current step: brewing cauldron -->
            <div class="absolute left-[-40px] top-1/2 -translate-y-1/2">
              <ProcessingIndicator />
            </div>
          {/if}
